...
df = spark.readStream.format("cloudFiles").option("cloudFiles.format", 'csv')\
    .option("cloudFiles.schemaLocation", '/dbfs/FileStore/tables/schema/flights.zip')\
    .load('/mnt/raw_datalake/flights.zip/') # this load specifies from which location(folder) we need to read data i.e. cloud container raw
    #option  # it will specify the location at which we need to store the schema at databricks 
    # in 1st option please check file format from Azure raw container
...
# Not cleaning flights data i.e. not performing any options on their columns just moving it to cleansd
...
df = spark.read.format('delta').load('/mnt/cleansed_datalake/flights')
...
f_delta_cleansed_load('flights', '/mnt/cleansed_datalake/flights',schema,'cleansed_geekcoders')

# COMMAND ----------

# MAGIC %sql
# MAGIC select * from cleansed_geekcoders.flights
